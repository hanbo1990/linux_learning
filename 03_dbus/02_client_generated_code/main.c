...
static void on_notify_name_owner(GObject *object, GParamSpec *pspec, gpointer user_data)
{
    ...
    g_print("name-owner changed to: %s\n", name_owner);
    g_free(name_owner);
}
...
gint main(gint argc, gchar *argv[])
{
    ...
    error = NULL;
    // GDBusObjectManagerClient is received from return if not NULL
    manager = example_object_manager_client_new_for_bus_sync(G_BUS_TYPE_SESSION,
                                                             G_DBUS_OBJECT_MANAGER_CLIENT_FLAGS_NONE,
                                                             "com.test.animal",
                                                             "/com/test/animal", NULL, /* GCancellable */
                                                             &error);
    if (manager == NULL) {
    ...
    // part of Property Details inside
    // https://developer.gnome.org/gio/stable/GDBusObjectManagerClient.html#GDBusObjectManagerClient--name-owner
    g_signal_connect(manager, "notify::name-owner", G_CALLBACK(on_notify_name_owner), NULL);
